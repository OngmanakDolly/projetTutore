...
{
    public function showProfilVacancier()
    {
        $profil=DB::select('select * from profiles where user_id = ?', [auth()->user()->id]);
        return view('profile/show', compact('profil'));
    }

    public function editProfilVacancier()
    {
        
        return view('profile/edit');
    }

    public function storeProfilVacancier(Request $request)
    {
...
public function showProfilOrganisme()
    {
        $profil=DB::select('select * from profiles where user_id = ?', [auth()->user()->id]);
        return view('profile/show', compact('profil'));
    }

    public function editProfilOrganisme()
    {
        
        return view('profile/edit');
    }

    public function storeProfilOrganisme(Request $request)
    {
        $domaine_d_activite = $request->domaine_d_activite;
        $competence = $request->competence;
        $experience_professionnelle = $request->experience_professionnelle;
    ...
    {
        $profil=DB::select('select * from profiles where user_id = ?', [$id]);
